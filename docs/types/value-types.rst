...
two fractional digits) as ``1.50``. In the EVM, it is stored in exactly the same way as the number ``150`` would be stored in an ``int8``.
...
.. note::
    ...
    that the number of bits used for the integer and the fractional part (the part after the decimal dot) is flexible in the former, while it is static
    in the latter. Generally, in floating point almost the entire space is used to represent the number, while only a small number of bits define
